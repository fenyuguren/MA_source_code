...
void Heu::JPA(vector<int>&InitialFamSeq, vector<vector<int>>&InitialJobinFamSeq,int &Cmax) {
	...
	for (int i = 0; i < this->m_Families; i++) {	
		...
		PartialFamSeq.clear();
		for (int j = m_Families - 19; j < m_Families; j++) {
			PartialFamSeq.push_back(FamSeq[j]);		
		}
		...
		int head = FamSeq[FamSeq.size() - 20];
		int tail = m_Families;
		...
		int start = 0;
		for (int j = m_Families - 19; j < m_Families; j++) {
			FamSeq[j]= AjustPartialFamSeq[start];
			start++;
		}
		fampop[i] = FamSeq;	
	}
...
int Heu::NEHFamwithTiebreak(vector<int> FamPrmuInFac, vector<vector<int>> JobSeqinFam, vector<int> &FamSeqInFac, int &SpanInFac)
{
	
	for (int i = 0; i < FamPrmuInFac.size(); i++)
	{
		int insertfam = FamPrmuInFac[i];
		int positions = FamSeqInFac.size() + 1;
		vector<vector<int>>fits;
		fits.clear();
		for (int pos = 0; pos < positions; pos++) {
			vector <int>TempFamSeq = FamSeqInFac;
			TempFamSeq.insert(TempFamSeq.begin() + pos, insertfam);			
			vector<vector<int>> JCTime(m_Jobs, vector<int>(m_Machines, 0));
			int Camx = GetJCTime_Forward_InFactory(TempFamSeq, JobSeqinFam, JCTime);
			int TFT = 0;
			for (int j = 0; j < m_Jobs; j++)
				TFT = TFT + JCTime[j][m_Machines - 1];
			fits.push_back({ Camx,TFT });
		}
		int bestpos = Tiebreaking(fits);
		FamSeqInFac.insert(FamSeqInFac.begin() + bestpos, insertfam);
	}
	SpanInFac=GetSpan(FamSeqInFac, JobSeqinFam);
	return SpanInFac;
}
