...
void IGPan::SetParameters(int d, double TempFactor, long TimeLimit)
{
	this->m_d = d;
	...
	while (true)
	{
		...
		if (m_Machines > 6) {
			...
			if (r < 0.1) {
				TempJobSeqinFam = JobSeqinFam;
				TempSpan = Span;
				LocalSearch_JobsInFac(FamSeq, TempJobSeqinFam, TempSpan);
				if (TempSpan < Span)
				{
					JobSeqinFam = TempJobSeqinFam;
					Span = TempSpan;
...
{
	ReadInstanceFileNameList("TS-III\\");
	...
	ostr_v1 << "MA_" << m_Families << "_" << CPUFactor << "_" << Rep << ".txt";
